...
use x402_client::X402Client;
use x402_tempo::TempoSchemeClient;
...
async fn main() {
    ...
    let base_url =
        std::env::var("RESOURCE_SERVER_URL").unwrap_or_else(|_| "http://localhost:4021".to_string());
    ...
    let signer: PrivateKeySigner = evm_private_key
        .parse()
        .expect("invalid EVM_PRIVATE_KEY");
