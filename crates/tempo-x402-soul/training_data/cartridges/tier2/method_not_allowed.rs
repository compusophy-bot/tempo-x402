...
pub extern "C" fn x402_handle(request_ptr: *const u8, request_len: i32) {
    ...
    host_log(0, "method_not_allowed: checking method");

    match method {
        "GET" => {
            respond(200, r#"{"message":"welcome","allowed_methods":["GET"],"docs":"/api/docs"}"#, "application/json");
        }
        "HEAD" => {
            respond(200, "", "application/json");
        }
        _ => {
            // Build 405 response with the rejected method name
            let mut pos = 0;
            pos = copy_to_scratch(pos, b"{\"error\":\"method not allowed\",\"method\":\"");
            pos = copy_to_scratch(pos, method.as_bytes());
            pos = copy_to_scratch(pos, b"\",\"allowed\":[\"GET\",\"HEAD\"],\"status\":405}");

            let result = unsafe { core::str::from_utf8_unchecked(&SCRATCH[..pos]) };
            respond(405, result, "application/json");
        }
    }
}
