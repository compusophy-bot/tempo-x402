...
/// Save cortex to database, retrying on lock errors.
pub fn save_cortex(db: &SoulDatabase, cortex: &Cortex) {
    ...
    while attempts < max_attempts {
        match db.set_state("cortex_state", &json) {
            Ok(_) => return,
            Err(e) => {
                let msg = format!("{:?}", e);
                if msg.contains("Resource temporarily unavailable") || msg.contains("code: 11") {
                    attempts += 1;
                    tracing::warn!(attempt = attempts, "Cortex save locked, retrying...");
                    std::thread::sleep(std::time::Duration::from_millis(50 * attempts));
                } else {
                    tracing::error!(error = %e, "Failed to save cortex state");
                    break;
                }
            }
        }
...
mod tests {
    ...
    fn test_causal_edges() {
        ...
        assert!(
            edge.as_ref().map(|e| e.observation_count).unwrap_or(0) >= 9,
    ...
    fn test_serialization() {
        ...
        let json = cortex.to_json();
        let restored = Cortex::from_json(&json).expect("Failed to restore Cortex from JSON");
