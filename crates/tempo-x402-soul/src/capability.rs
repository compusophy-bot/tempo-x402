...
use crate::validation::{PlanViolation, Severity, ValidationResult};
use x402_cartridge::manifest::CartridgeManifest;
...
impl Capability {
    ...
    /// Validate a cartridge manifest.
    pub fn test_cartridge_manifest(manifest: &CartridgeManifest) -> ValidationResult {
        let mut violations = Vec::new();
        if manifest.name.is_empty() {
            violations.push(PlanViolation {
                rule: "missing-name",
                severity: Severity::Hard,
                detail: "Cartridge name is required".to_string(),
                step_index: None,
            });
        }
        if manifest.slug.is_empty() {
            violations.push(PlanViolation {
                rule: "missing-slug",
                severity: Severity::Hard,
                detail: "Cartridge slug is required".to_string(),
                step_index: None,
            });
        }
        ValidationResult {
