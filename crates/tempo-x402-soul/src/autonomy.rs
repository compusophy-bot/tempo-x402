...
use crate::synthesis::{self, SafeDb, CognitiveState};
...
pub fn compile_autonomous_plan(
    ...
    // Revert to original Mutex logic for now to avoid the OS lock issues
    let _db_lock = match safe_db.try_lock() {
        Some(lock) => lock,
        None => {
            tracing::warn!("Database contention in autonomy, retrying...");
            std::thread::sleep(std::time::Duration::from_millis(100));
            match safe_db.try_lock() {
                Some(lock) => lock,
                None => {
                    tracing::error!("Database still locked after retry");
                    return CompilationResult::FallbackToLlm("Database locked by another process".to_string());
                }
            }
        }
    };
...
pub fn diagnose_cognition(db: &SoulDatabase) -> Vec<ImprovementGoal> {
    ...
    if synth.state == crate::synthesis::CognitiveState::Stuck {
        goals.push(ImprovementGoal {
    ...
    if synth.state == crate::synthesis::CognitiveState::Conflicted {
        goals.push(ImprovementGoal {
...
mod tests {
    ...
    fn test_diagnose_cognition_empty() {
        ...
        let path = format!("/tmp/test_diagnose_{}", uuid::Uuid::new_v4());
        let db = crate::db::SoulDatabase::new(&path).expect("Failed to create soul DB");
        let goals = diagnose_cognition(&db);
        assert!(
            !goals.is_empty(),
    ...
    fn test_improvement_prompt() {
        let db = crate::robust_db::RobustDbManager::open(":memory:").expect("Failed to create memory DB");
    ...
    fn test_compile_fallback() {
        use std::sync::Arc;
        let path = format!("/tmp/test_db_{}", uuid::Uuid::new_v4());
        let db = Arc::new(SoulDatabase::new(&path).unwrap());
        let safe_db = SafeDb::new(db.clone());
        let result = compile_autonomous_plan(&safe_db, "goal-1", "some random goal", "agent-1");
    ...
    fn test_compile_with_template() {
        use std::sync::Arc;
        let path = format!("/tmp/test_db_{}", uuid::Uuid::new_v4());
        let db = Arc::new(SoulDatabase::new(path.as_str()).unwrap());
        let safe_db = SafeDb::new(db.clone());
        let result = compile_autonomous_plan(&safe_db, "goal-1", "Fix compile error in brain.rs", "agent-1");
        match result {
            CompilationResult::Compiled(plan) => {
