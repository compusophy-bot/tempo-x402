...
mod tests {
    use super::*;
    use crate::robust_db::RobustDbManager;

    #[test]
    fn test_compute_free_energy() {
        let db = RobustDbManager::open(":memory:").expect("failed to create memory db for test_compute_free_energy");
        let fe = compute(&db);
    ...
    fn test_regime_detection() {
        let db = RobustDbManager::open(":memory:").expect("failed to create memory db for test_regime_detection");
        let fe = compute(&db);
    ...
    fn test_measure_and_history() {
        let db = RobustDbManager::open(":memory:").expect("failed to create memory db for test_measure_and_history");
    ...
    fn test_components_sum() {
        let db = RobustDbManager::open(":memory:").expect("failed to create memory db for test_components_sum");
        let fe = compute(&db);
    ...
    fn test_prompt_section() {
        let db = RobustDbManager::open(":memory:").expect("failed to create memory db for test_prompt_section");
    ...
    fn test_serialization() {
        ...
        let json = serde_json::to_string(&fe).expect("failed to serialize free energy");
        let restored: FreeEnergy = serde_json::from_str(&json).expect("failed to deserialize free energy");
