...
pub(super) async fn soul_status(state: web::Data<NodeState>) -> HttpResponse {
    ...
    let active_plan = soul_db.get_active_plan().ok().flatten().map(|p| {
        let _current_step_type = p.steps.get(p.current_step).map(|s| s.summary());
        let ctx = if p.context.is_empty() {
            None
        } else {
            Some(serde_json::to_string(&p.context).unwrap_or_default())
        };
        PlanInfo {
            id: p.id,
            goal_id: Some(p.goal_id),
            current_step: p.current_step.to_string(),
            total_steps: p.steps.len(),
            status: p.status.as_str().to_string(),
            replan_count: p.replan_count,
            current_step_type: p.steps.get(p.current_step).map(|s| s.summary()).unwrap_or_default(),
    ...
    let pending_plan = soul_db.get_pending_approval_plan().ok().flatten().map(|p| {
        let goal_desc = soul_db
            ...
            .flatten()
            .map(|g| g.description);
        let _step_summaries: Vec<String> = p.steps.iter().map(|s| s.summary()).collect();
        let ctx = if p.context.is_empty() {
            None
        } else {
            Some(serde_json::to_string(&p.context).unwrap_or_default())
        ...
        PlanInfo {
            ...
            steps: Some(p.steps.iter().map(|s| s.summary()).collect()),
            context: ctx,
